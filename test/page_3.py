...
def start_interview():
    ...
    if st.button("🚀 Start Interview", type="primary", use_container_width=True):
        ...
        if question:
            ...
            st.rerun()  # TODO: why rerun here? like there is nothing we are doing here which will reflect to ui
    ...
    with col1:
        if st.button("🔄 Start New Interview", type="primary"):
            ...
            for key in list(st.session_state.keys()):
                if isinstance(key, str) and key.startswith(('interview_', 'current_', 'total_', 'is_', 'audio_', 'questions_')):
...
def main():
    ...
    if 'email' not in st.session_state:
        st.session_state.email = "rajrjpushp@gmail.com"  # Default for testing
